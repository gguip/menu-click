...
- **S30 — Toda conta precisa de caminho de volta.** O projeto passou um tempo com um buraco sério: o restaurante tinha exatamente um login (criado no `/auth/register`), nenhuma rota de troca de senha e nenhuma de recuperação. Quem esquecesse a senha **perdia o restaurante** — o único conserto era `UPDATE` na mão no banco. Hoje existem `POST /auth/change-password` e o CRUD de usuários; ao mexer em autenticação, pergunte sempre "e se a pessoa perder a credencial?" antes de considerar o fluxo pronto.
...
Ainda **não** existe, e ao implementar vale desde o primeiro commit: **recuperação de senha por e-mail** (exige serviço de envio, que é dependência e infra novas — é o que falta para quem perde a senha do último `owner`) e permissão por ação, mais fina que os dois papéis de hoje.
